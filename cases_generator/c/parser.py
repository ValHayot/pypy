...
from transpyle.general import Parser, CodeWriter, run_tool, CodeReader
...
class C99Preprocessor(pcpp.Preprocessor):
    ...
    def include(self, tokens: t.List[LexToken]) -> t.List[LexToken]:
        ...
        self.i += 1
        tokens[0].value = 'const char* directive_{} = "#include{}'.format(
            self.i, tokens[0].value
        )
        # for token in tokens:
    ...
    def on_directive_unknown(self, directive, toks, ifpassthru):
        ...
        if directive.value == "warning":
            _LOG.warning(
                ...
                directive.source,
                directive.lineno,
                "".join(tok.value for tok in toks),
            )
            return True
        return None
    ...
    def _parse_scope(self, code: str, path: pathlib.Path = None):
        ...
        if includes:
            ...
            run_tool(
                ...
                [
                    "-I",
                    str(TRANSPYLE_C_RESOURCES_PATH),
                    "-o",
                    output_path,
                    "-E",
                    intermediate_path,
